...
// Constants:
static const float kObjectScale = 160.f;

QCAR::DataSet* dataSetStonesAndChips    = 0;
QCAR::DataSet* dataSetThrone            = 0;

bool switchDataSetAsap          = false;

JavaVM* gJavaVM = NULL;
jclass gActivityClass;
jobject gActivityObj;
...
class ImageTargets_UpdateCallback : public QCAR::UpdateCallback
{
    virtual void QCAR_onUpdate(QCAR::State& /*state*/)
    {
        if (switchDataSetAsap)
        {
            ...
            if (imageTracker == 0 || dataSetStonesAndChips == 0 || dataSetThrone == 0 ||
            ...
            if (imageTracker->getActiveDataSet() == dataSetStonesAndChips)
            {
                ...
                imageTracker->activateDataSet(dataSetThrone);
            }
            else
            {
                imageTracker->deactivateDataSet(dataSetThrone);
...
Java_com_qualcomm_QCARSamples_ImageTargets_ImageTargets_loadTrackerData(JNIEnv *, jobject)
{
    ...
    dataSetThrone = imageTracker->createDataSet();
    if (dataSetThrone == 0)
    {
        LOG("Failed to create a new tracking data.");
        return 0;
    ...
    // Load the data sets:
    if (!dataSetStonesAndChips->load("throne.xml", QCAR::DataSet::STORAGE_APPRESOURCE))
    {
        LOG("Failed to load data set.");
        return 0;
    }

    if (!dataSetThrone->load("throne.xml", QCAR::DataSet::STORAGE_APPRESOURCE))
    {
        LOG("Failed to load data set.");
        return 0;
    }
...
Java_com_qualcomm_QCARSamples_ImageTargets_ImageTargets_destroyTrackerData(JNIEnv *, jobject)
{
    ...
    if (dataSetThrone != 0)
    {
        if (imageTracker->getActiveDataSet() == dataSetThrone &&
            !imageTracker->deactivateDataSet(dataSetThrone))
        {
        ...
        if (!imageTracker->destroyDataSet(dataSetThrone))
        {
...
JNIEXPORT int JNICALL
Java_com_qualcomm_QCARSamples_ImageTargets_ImageTargets_countTrackingNative(JNIEnv *, jobject)
{
	 QCAR::State state = QCAR::Renderer::getInstance().begin();

	return state.getNumActiveTrackables();
}
...
//    glEnable(GL_CULL_FACE);
    ...
    if (state.getNumActiveTrackables() == 0) {
    	hidden = 1;
		LOG("hide buttons calling");
		JNIEnv *env;
		gJavaVM->AttachCurrentThread(&env, NULL);
		jmethodID method = env->GetMethodID(gActivityClass, "hideButtons", "()V");
		env->CallVoidMethod(gActivityObj, method);
		LOG("hide buttons called");
    }
    ...
    for(int tIdx = 0; tIdx < state.getNumActiveTrackables(); tIdx++)
    {
        ...
        glTexCoordPointer(2, GL_FLOAT, 0, (const GLvoid*) &FrostSwordTexCoords[0]);//teapotTexCoords[0]);
        glVertexPointer(3, GL_FLOAT, 0, (const GLvoid*) &FrostSwordVerts[0]);//teapotVertices[0]);
        glNormalPointer(GL_FLOAT, 0,  (const GLvoid*) &FrostSwordNormals[0]);//teapotNormals[0]);
        glDrawArrays(GL_TRIANGLES, 0,FrostSwordNumVerts);
#else
        ...
        glVertexAttribPointer(vertexHandle, 3, GL_FLOAT, GL_FALSE, 0,
                              (const GLvoid*) &FrostSwordVerts[0]);//teapotVertices[0]);
        glVertexAttribPointer(normalHandle, 3, GL_FLOAT, GL_FALSE, 0,
                              (const GLvoid*) &FrostSwordNormals[0]);//teapotNormals[0]);
        glVertexAttribPointer(textureCoordHandle, 2, GL_FLOAT, GL_FALSE, 0,
                              (const GLvoid*) &FrostSwordTexCoords[0]);//teapotTexCoords[0]);
        ...
        if (hidden) {
        	JNIEnv *env;
			gJavaVM->AttachCurrentThread(&env, NULL);
			jmethodID method = env->GetMethodID(gActivityClass, "showButtons", "()V");
			env->CallVoidMethod(gActivityObj, method);
        }
                            ...
                            JNIEnv* env, jobject obj, jint width, jint height)
{
    ...
    jmethodID getTextureCountMethodID = env->GetMethodID(gActivityClass,
                                                    "getTextureCount", "()I");
    ...
    jmethodID getTextureMethodID = env->GetMethodID(gActivityClass,
